...
function setUnit(unit) {
  ...
  if (currentWeatherData !== null) {
    ...
    // RE-FETCH or RE-DISPLAY Forecast to update its units
    // Since we don't want to call the API again,
    // it's best to store the forecast data in a variable like 'currentForecastData'
    // and just call displayForecast(currentForecastData) here.
    if (lastSearchedCity) fetchForecast(lastSearchedCity);
  }
}
...
async function fetchForecast(city) {
  try {
    ...
    // 1. Group the 3-hour forecasts by date
    const dailyData = {};
    ...
    // 2. Process each day to find true High, Low, and a representative Icon
    const formattedForecast = Object.keys(dailyData).map((date) => {
      const dayItems = dailyData[date];

      // Calculate max and min by looking at every 3-hour block in that day
      const tempMax = Math.max(...dayItems.map((i) => i.main.temp_max));
      const tempMin = Math.min(...dayItems.map((i) => i.main.temp_min));

      // Pick an icon: Try to get the one at 12:00, otherwise pick the middle one
      const midDayItem =
        dayItems.find((i) => i.dt_txt.includes("12:00:00")) ||
        dayItems[Math.floor(dayItems.length / 2)];

      return {
        date: dayItems[0].dt, // Unix timestamp
        tempMax,
        tempMin,
    ...
    // 3. Slice to get the next 5 days (excluding today if you wish, or keep it)
    // Most users prefer seeing the next 5 days starting from tomorrow
    displayForecast(formattedForecast.slice(1, 6));
  } catch (error) {
...
function displayForecast(forecastList) {
  ...
  forecastList.forEach((item) => {
    ...
    const emoji = getWeatherEmoji(item.icon);

    // Convert temps based on the current globally selected unit
    const high = convertTemp(item.tempMax, currentUnit);
    const low = convertTemp(item.tempMin, currentUnit);
...
function updateSunPosition(sunriseTimestamp, sunsetTimestamp) {
  ...
  // 1. Determine if it is currently day or night
  const isDaylight = now >= sunriseTimestamp && now <= sunsetTimestamp;

  if (!isDaylight) {
    // Hide the sun dot or place it "below the horizon"
    sunDot.style.opacity = "0";
    arcFill.style.strokeDashoffset = "283"; // Assuming 283 is the total path length
    return;
  }

  // 2. Show the sun and calculate progress (0 to 1)
  sunDot.style.opacity = "1";
  const totalDaylight = sunsetTimestamp - sunriseTimestamp;
  const elapsed = now - sunriseTimestamp;
  const progress = elapsed / totalDaylight; // 0 at sunrise, 1 at sunset

  /*
     3. Calculate Coordinates
     X: Move from 5% to 95% (to prevent edge clipping)
     Y: We use a sine wave.
        Math.sin(0) is 0, Math.sin(PI/2) is 1, Math.sin(PI) is 0.
  */
  const xPercent = 5 + progress * 90;

  // We multiply by 80 and add 10 to keep the sun between 10% and 90% height
  // This prevents the sun from touching the very top or bottom edges
  const yPercent = 90 - Math.sin(progress * Math.PI) * 80;

  sunDot.style.left = `${xPercent}%`;
  sunDot.style.top = `${yPercent}%`;

  /*
     4. Arc Fill Animation
     If your SVG path length is 283:
     0 offset = fully colored (sunset)
     283 offset = empty (sunrise)
  */
